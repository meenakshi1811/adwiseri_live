...
?>
    ...
    public function get_application(Request $request)
    {
        ...
        if ($comm === 'invoice' && $subscriber) {
            ...
            $excludeApplicationIds = app(InvoiceItemService::class)->invoicedApplicationRecordIdsForClient(
                (int) $subscriber->id,
                (int) $id,
                $ignoreInvoiceId > 0 ? $ignoreInvoiceId : null
            );
    ...
    public function new_invoice()
    {
        ...
        $clients = Clients::where('subscriber_id', '=', $subscriber->id)->orderBy('name')->get()
            ->filter(fn ($client) => $this->invoiceItemService()->clientHasUninvoicedApplications((int) $subscriber->id, (int) $client->id))
            ->values();
        if (count($clients) < 1) {
    ...
    private function persistInvoiceItems(Internal_Invoices $invoice, Request $request, ?int $clientId = null, ?int $ignoreInvoiceId = null): array
    {
        $itemService = $this->invoiceItemService();
        $items = $itemService->normalizeRequestItems($request);
        $itemService->assertHasItems($items);

        if ($clientId && $invoice->type === 'ar') {
    ...
    public function create_new_invoice(Request $request)
    {
        ...
        if ($user) {
            ...
            if ($request->client) {
                ...
                $this->persistInvoiceItems($invoice, $request, (int) $request->client, null);
    ...
    public function email_broadcast()
    {
        ...
        return view('web.email_broadcast', compact('user', 'page', 'staffMembers', 'clients', 'subscriber', 'subscriberFooter'));
    }
    ...
    private function notifyAppointmentResponse(
        ...
        try {
            if (!empty($sender->email)) {
                Mail::to($sender->email)->send(new AppointmentResponseMail($appointment, $client, $sender, $response));
            }
        } catch (\Throwable $e) {
