@php
    ...
    $hasSiteUsers = count($siteusers ?? []) > 0;
@endphp
...
<div class="row m-0 pb-2 module-tab-row">
    ...
    <div class="col-3 border p-1 text-center tab-anchor {{ $activeTab === 'documents' ? 'bg-info text-white' : 'top_modules' }}"
        @if($activeTab !== 'documents') onclick="window.location.href = '{{ route('client_documents') }}';" @endif>
        Documents
    </div>
    <div class="col-3 border p-1 text-center tab-anchor {{ $activeTab === 'management' ? 'bg-info text-white' : 'top_modules' }}"
        @if($activeTab !== 'management' && $user->user_type == 'Subscriber') onclick="window.location.href = '{{ route('user_applications') }}';" @endif>
    ...
    @else
    <div class="col-3 border p-1 text-center tab-anchor top_modules" id="app_tracking_zero" style="cursor:pointer;">
        Application Tracking
    </div>
    @endif
</div>
