...
<div class="col-lg-10 column-client">
    <div class="client-dashboard">
        <div class="client-btn d-flex mb-2">
            <h3 class="text-primary text-center flex-grow-1 m-0">Add Client Account Record</h3>
        </div>

        <div class="col">
            <form id="client_account_form" class="register-box login-box" method="POST" action="{{ route('store_client_account') }}">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-8 p-1">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="trans_type" id="trans_credit" value="Credit" {{ old('trans_type', 'Credit') === 'Credit' ? 'checked' : '' }}>
                            <label class="form-check-label" for="trans_credit">Credit</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="trans_type" id="trans_debit" value="Debit" {{ old('trans_type') === 'Debit' ? 'checked' : '' }}>
                    ...
                    <div class="col-md-8 p-1">
                        ...
                        <input name="description_other" id="description_other" type="text" maxlength="255"
                            ...
                            value="{{ old('description_other') }}" placeholder="Specify other description"
                            style="display: {{ old('description') === 'Other' ? 'block' : 'none' }};">
                        @error('description')
                            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                        @enderror
                        @error('description_other')
                    ...
                    <div class="col-md-8 p-1">
                        <input name="transaction_date" type="text" required
                            ...
                            value="{{ old('transaction_date', date('d-m-Y')) }}" placeholder="DD-MM-YYYY">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var initialDescription = @json(old('description', ''));
        ...
        fetch('{{ url('/get-applications-by-client') }}/' + clientId)
            ...
            .then(function (apps) {
                apps.forEach(function (app) {
                    ...
                    if (selectedId && String(selectedId) === String(app.id)) {
                        option.selected = true;
                    }
    ...
    var oldClientId = @json(old('client_id'));
    var oldApplicationId = @json(old('application_id'));
    if (oldClientId) {
        loadApplications(oldClientId, oldApplicationId);
    }

    var form = document.getElementById('client_account_form');
    var debitOverCreditConfirmed = false;

    function submitFormAfterConfirm() {
        debitOverCreditConfirmed = true;
        form.submit();
    }

    function showDebitOverCreditPrompt() {
        if (typeof Swal === 'undefined') {
            if (window.confirm('Are you sure?\n\nDebit amount is more than the credit on client\'s account.')) {
        ...
        Swal.fire({
            icon: 'warning',
            title: 'Are you sure ?',
            text: "Debit amount is more than the credit on client's account.",
    ...
    form.addEventListener('submit', function (event) {
        if (debitOverCreditConfirmed) {
            debitOverCreditConfirmed = false;
            return;
        }

        if (getTransType() !== 'Debit') {
            return;
        }
        ...
        var params = new URLSearchParams({ client_id: clientId });
        if (applicationId) {
            params.append('application_id', applicationId);
        }

        fetch('{{ route('client_account_balance') }}?' + params.toString(), {
            ...
            .then(function (data) {
                var availableCredit = parseFloat(data.balance);
                if (isNaN(availableCredit)) {
                    availableCredit = 0;
                }

                if (amount > availableCredit) {
                    showDebitOverCreditPrompt();
                } else {
                    submitFormAfterConfirm();
                }
            })
