...
@section('main-section')
    ...
    <div class="col-lg-10 column-client">
        <h3 class="text-primary text-center px-2">Add New Business Entry</h3>
        <div class="col">
            <form class="register-box login-box" method="POST" action="{{ route('store_associate_business') }}" autocomplete="off">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-8 p-1">
                        <select name="application_id" id="applicationSelect" class="form-control form-select @error('application_id') is-invalid @enderror" required>
                            ...
                            <option value="{{ $app->id }}" data-client-id="{{ $app->client_id }}" data-status="{{ $app->application_status }}" {{ old('application_id') == $app->id ? 'selected' : '' }}>{{ $app->application_name }} ({{ $app->application_id }})@if($app->client_name) — {{ $app->client_name }}@endif</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-8 p-1">
                        <input name="fees" type="number" step="0.01" min="0" class="form-control @error('fees') is-invalid @enderror" value="{{ old('fees') }}" required placeholder="Fees">
                    ...
                    <div class="col-md-8 p-1">
                        <input type="text" id="applicationStatusDisplay" class="form-control @error('application_status') is-invalid @enderror" readonly placeholder="Select an application" value="{{ old('application_status') }}">
                        <input type="hidden" name="application_status" id="applicationStatusInput" value="{{ old('application_status') }}">
...
<script>
    $(document).ready(function(){
        ...
        // Keep a pristine copy of every application option to rebuild the list from.
        ...
        // Rebuild the Application list to only those belonging to the chosen client
        // and not already linked to any associate.
        function filterApplications(){
            ...
            // Preserve the previous choice if it is still valid for this client.
            if(selected && $app.find('option[value="'+selected+'"]').length){
                $app.val(selected);
            }
        }

        // Auto-fill Application Status from the linked application's DB value.
