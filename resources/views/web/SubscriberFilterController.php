...
class SubscriberFilterController extends Controller
{
    ...
    public function subscribersReport()
    {
        ...
        if (request()->type == "country") {
            ...
            // NOTE: Show each outstanding receivable (AR) separately, one bar per invoice/service.
            // Do NOT filter on application_id — most receivables are recorded against an
            // invoice / service_description with no linked application (application_id NULL),
            // and filtering them out is what made this chart appear empty.
            $applicationOutstandingAmount = $query->whereBetween('payment_ar.created_at', [$startDate, $endDate])
                ->where('payment_ar.type', 'ar') // receivables only (amounts owed by clients)
                ->join('clients', 'clients.id', '=', 'payment_ar.client_id') // ✅ Properly join clients table
                ->leftJoin('applications', 'applications.id', '=', 'payment_ar.application_id') // ✅ Link application when present
                ->select(
                    'payment_ar.client_id',
                    'payment_ar.application_id',
                    'payment_ar.invoice_no',
                    'clients.name as client_name', // ✅ Properly select client name
                    // Label = "Client name - Application/Service Name". Fall back to the
                    // service description, then the invoice no. when no application is linked.
                    DB::raw("CONCAT(
                                clients.name,
                                ' - ',
                                COALESCE(
                    ...
                    DB::raw('MAX(payment_ar.created_at) as created_at'),
                    // Outstanding for the invoice = invoice total - everything paid against it.
                    // amount is repeated across partial payments, so use MAX(amount) (not SUM)
                    // to avoid double-counting the total.
